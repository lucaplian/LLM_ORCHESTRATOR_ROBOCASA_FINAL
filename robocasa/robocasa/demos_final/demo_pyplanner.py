...
import argparse
import robocasa
from robocasa.demos_final.demo_pytree import (
    ...
    OpenFridgeAction,
    CloseFridgeAction,
    PickEggFromFridgeAction,
    PutEggInBowlAction,
    TurnOnStoveAction,
    BoilWaterAction,
    PickEggFromBowlAction,
    PutEggInPotAction,
    BoilEggsAction,
    TurnOffStoveAction,
    PickEggOutOfPotAction,
...
def generate_llm_command_for_prompt(args, actions_spec, actions):
    ...
    from robocasa.demos_final.demo_pytree import find_the_initial_world_state
    actions = actions[:22]
    dict_actions_ = {}
    for i, action in enumerate(actions):
        if "egg2" in action:
            action = action.replace("egg2_", "egg_")
        elif "egg1" in action:
            action = action.replace("egg1_", "egg_")
        
        if "egg" in action:
            if action not in dict_actions_.keys():
                dict_actions_[action] = 1
            else:
                dict_actions_[action] = dict_actions_[action] + 1
            
            action += f" ({dict_actions_[action]})"
    ...
    if not args.do_it_as_json:
        ...
        if not args.execute_the_parse_for_the_class:
            ...
            while try_again == 1:
                ...
                try:
                    ...
                    bt_start = time.time()
                    os.system(f"mjpython -m robocasa.demos_final.demo_pytree --custom_goal_state --custom_world_state --custom_initial_state --action_list {action_list_response} --time {start_time_T1}")
            ...
            IMPORTANT_IMPORTS = f"""import robocasa\nfrom py_trees.behaviour import Behaviour\nfrom py_trees.common import Status\nfrom py_trees.composites import Sequence\nfrom py_trees import logging as log_tree\nfrom robocasa.demos_final.demo_pytree import (  OpenCabinetDoorAction, PickPotFromCabinetAction, PutPotInSinkAction, TurnOnSinkAction, PositionFaucetInPotAction, PutWaterInPotAction, RepositionFaucetAction, TurnOffSinkAction, PickPotFromSinkAction, PutPotInStoveAction, OpenFridgeAction, PickEggFromFridgeAction, CloseFridgeAction, CloseFridgeAction, PutEggInBowlAction, TurnOnStoveAction, BoilWaterAction, PickEggFromBowlAction, PutEggInPotAction, BoilEggsAction, TurnOffStoveAction, PickEggOutOfPotAction, PickPotFromStoveAction, SpillWaterAction, PutPotInCabinetAction, CloseDoorCabinetAction, Condition,  reset, get_dict_enum, custom_initial_state_definition, initialize_custom_goal_state, initialize_custom_world_state)\nreset()\nget_dict_enum()\ncustom_initial_state_definition(time={start_time_T1})\ninitialize_custom_goal_state(time={start_time_T1})\ninitialize_custom_world_state(time={start_time_T1})\n
            """
            total_tokens = 0
            correction_rounds = 0
            planning_start = time.time()
            while True:
                ...
                #CRITICAL: DO NOT PUT ANY SORT OF PARAMETERS INSIDE THE CLASSES. A CLASS ACTION detects automatically which object should be put
                """
                ...
                CONTEXT_WORD = f"""
                ? represents a placeholder for a number of a specific object instance, but please 
                ...
                if args.optimize or args.hard_optimize:
                    CONTEXT_WORD+="CRITICAL: FIND THE MOST OPTIMAL SEQUENCE OF CLASSES POSSIBLE, WITH THE LEAST POSSIBLE AMOUNT OF REDUNDANCIES AND DUPLICATES."
                    if args.hard_optimize:
                        CONTEXT_WORD+="For example, instead of putting the eggs in the bowl after picking the egg from the fridge or any other place, TRY to put them in the pot directly, but make sure that the preconditions and effects are suitable for this."
                ...
                if all_previous_non_optimal_codes != []:
                    LLM_PROMPT2+=f'''
                    These are previous unoptimal classes that repeated unnecessarily, take into consideration this, when you make a new one:
                    {all_previous_failed_codes}
                    '''
                ...
                if try_attempts <=5:
                    ...
                    if args.optimize or args.hard_optimize:
                        second_tool_definition["description"] += "In addition, try to verify if the current path contains redundancies (unnecessary duplicate Classes)."
                        second_tool_definition["parameters"]["properties"]["is_an_optimal_path"] = {
                                "type": "boolean",
                                "description": "It shows whether a list of classes is optimal or not."
                        }
                        second_tool_definition["parameters"]["properties"]["list_of_non_optimal_actions"] = {
                                "type": "array",
                                "items": {"type": "string"},
                                "description": "Show the list of Classes that the robot takes, excluding the ones the robot is obliged to take, due to the constraints at the beginning of the environment."
                        }
                    ...
                    context = []
                    context.append(f'? represents a placeholder for a number of a specific object instance')
                    ...
                    if function_call_second["reached_goal_state"] == True:
                        ...
                        try: 
                            ...
                            bt_start = time.time()
                            os.system(f"mjpython -m robocasa.demos_final.parse_classes")
                            bt_time = time.time() - bt_start
                            print(f"Running time BT: {bt_time:.2f}s")
                        ...
                        all_previous_non_optimal_codes.append(f"{action_list_}. These actions were duplicated in particular {duplicated_actions}")
        ...
        while True:
                ...
                "parameters": {
                    ...
                    "properties": {
                        "action_list": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "List of action names, make sure to never place a number of an instance when calling a specific action, only mention the instance as a whole."
                        },
                        "goal_state": {
                            "type": "object",
            ...
            context = []
            context.append("? represents a placeholder for a number of a specific object instance\n")
            context.append(f'WARNING: YOU MUST VERIFY IN-DEPTH THE "beginning_world_state". Do not assume the a parameter without consulting the "beginning_world_state", and start with that when you verify certain actions.')
            context.append(f'CRITICAL: CHECK THE LOCATIONS OF OBJECTS (e.g. do not assume that the pot is in the cabinet, or that the pot is in the sink), and take into consideration the states of the environment (e.g. the sink is turned on)')
            ...
            context.append(f'For "available_actions" key, each action contains precondities, which need to be fullfiled before the triggering of so action, and and effects, which results by triggering said actions. You need to verifying every precondition, and verify properly if one is the cause of not reaching the final goal state')
            context.append(f'For "list_of_actions" key, represents the actions done by the action planner in order to reach the goal state, you need to verify each carefully, taking into consideration the preconditions and their effect on the environment states. In addition, make sure to never place a number of an instance when calling a specific action, only mention the instance as a whole.')
            second_json_arguments["context"] = context
            second_json_arguments = json.dumps(second_json_arguments, indent=2)
            second_input_list = [
                {"role": "user", "content": f"{second_json_arguments}"}
        ...
        bt_start = time.time()
        os.system(f"mjpython -m robocasa.demos_final.demo_pytree --custom_goal_state --custom_world_state --custom_initial_state --action_list {list_act} --time {start_time_T1}")
        bt_time = time.time() - bt_start
        print(f"Running time BT: {bt_time:.2f}s")
        return
...
if __name__ == "__main__":
    actions_spec = {
        ...
        "pick_egg_from_fridge": {
            "preconditions": {
                "is_fridge_closed": False,
                "is_fridge_opened": True,
                "egg?_location":  "FRIDGE",
                "is_robot_grip_occupied": False
            },
            "effects": {
                "egg?_location": "ROBOT_HAND",
                "is_robot_grip_occupied": True
            },
            "description": "picks the egg? from the fridge, ? represents the placeholder for a number"
        },
        ...
        "put_egg_in_bowl": {
            ...
            "description": "puts the egg? in the bowl, ? represents the placeholder for a number"
        },
        ...
        "put_egg_in_pot": {
            "preconditions": {
                "egg?_location": "ROBOT_HAND",
                "is_robot_grip_occupied":  True,
                "is_pot_occupied_egg?": False,
                "is_water_boiled": True
            },
            "effects": {
                "is_robot_grip_occupied":  False,
                "egg?_location": "POT",
                "is_pot_occupied_egg?": True


            },
            "description": "it puts the egg? inside the pot, ? represents the placeholder for a number"
        },
        "pick_egg_from_bowl": {
            "preconditions": {
                "egg?_location": "BOWL",
            ...
            "description": "it picks the egg? from the bowl, where ? represents the placeholder for a number"
        ...
        "pick_egg_out_of_pot": {
            ...
            "description": "picks the egg? out of the pot, where ? represents the placeholder for a number"
