...
def determine_robot_egg1_grip(env, name="", ignore_states=False, state = None):
  ...
  if (finger_joint1 > 0.01 and finger_joint1 < 0.03) and (finger_joint2 > 0.01 and finger_joint2 < 0.03):
    if (diff_x_axis_egg1 < 0.05 and diff_y_axis_egg1 < 0.05 and diff_z_axis_egg1 < 0.05):
      print("ignore_states=", ignore_states)
      if ignore_states == False:
        robocasa.demos.singleton.world_state["is_robot_grip_occupied"] = True
...
def determine_robot_egg2_grip(env, name="", ignore_states=False, state = None):
  ...
  diff_x_axis_egg2 = abs(env.sim.data.get_site_xpos("gripper0_right_grip_site")[0] - env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]])
  diff_y_axis_egg2 = abs(env.sim.data.get_site_xpos("gripper0_right_grip_site")[1] - env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]+1])
  diff_z_axis_egg2 = abs(env.sim.data.get_site_xpos("gripper0_right_grip_site")[2] - env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]+2])
  finger_joint1 = abs(env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("gripper0_right_finger_joint1")])
  finger_joint2 = abs(env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("gripper0_right_finger_joint2")])
  #global world_state
  if name != "":
...
def determine_fridge_state(env, name=""):
  #data = robocasa.demos.singleton.world_state["egg2_location"].name
  #print("data=???", data)
...
def determine_nr_eggs_fridge(env, name):
  egg1_1 = env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg1_joint0")[0]]
  egg1_2 = env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg1_joint0")[0]+1]
  egg1_3 = env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg1_joint0")[0]+2]

  egg2_1 = env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]]
  egg2_2 = env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]+1]
  egg2_3 = env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]+2]
  ...
  is_okay_egg2_1 =  (egg2_1 >= env.fridge.pos[0] - env.fridge.size[0]/2 + 0.01 and egg2_1 <= env.fridge.pos[0] + env.fridge.size[0]/2 - 0.01)
  is_okay_egg2_2 =  (egg2_2 >= env.fridge.pos[1] - env.fridge.size[1]/2 + 0.01 and egg2_2 <= env.fridge.pos[1] + env.fridge.size[1]/2 - 0.01)
  is_okay_egg2_3 =  (egg2_3 >= env.fridge.pos[2] - env.fridge.size[2]/2  and egg2_3 <= env.fridge.pos[2] + env.fridge.size[2]/2)

  egg1_final_okay = is_okay_egg1_1 and is_okay_egg1_2 and is_okay_egg1_3
  egg2_final_okay = is_okay_egg2_1 and is_okay_egg2_2 and is_okay_egg2_3
  number_okay = 0
  if egg1_final_okay:
    number_okay+=1
  if egg2_final_okay:
...
class PickEgg2FromFridgeAction(Behaviour):
  def __init__(self):
    super().__init__("pick_egg2_from_fridge")
    self.conditions = [
      Condition("is_robot_grip_not_occupied", "pick_egg2_from_fridge"),
      Condition("check_eggs_1", "pick_egg2_from_fridge"),
      Condition("is_fridge_egg2_there", "pick_egg2_from_fridge")
    ]
  def setup(self):
    self.logger.debug(f"PickEgg2FromFridgeAction::setup")

  def initialise(self):
    self.logger.debug(f"PickEgg2FromFridgeAction::initialise")
  ...
  def update(self):
    ...
    robocasa.demos.singleton.general_information["previous_state"] = final_state
    robocasa.demos.singleton.general_information["previous_env"] = env
    if determine_robot_egg2_grip(env, "is_egg2_in_robot_grip") == Status.SUCCESS:
      return determine_egg_fridge(env, "is_not_fridge_egg2_there")
    return Status.FAILURE

  
  def terminate(self, new_status):
    self.logger.debug(f"PickEgg2FromFridgeAction::terminate to {new_status}")
...
class CloseFridgeAction(Behaviour):
  def __init__(self):
    ...
    print("robocasa.demos.singleton.world_state[egg2_location]=", robocasa.demos.singleton.world_state["egg2_location"])    
    self.conditions = [
  ...
  def initialise(self):
    #value = robocasa.demos.singleton.world_state["egg2_location"].name
...
class PutEgg2InBowlAction(Behaviour):
  def __init__(self):
    super().__init__("put_egg2_in_bowl")
    self.conditions = [
      Condition("is_egg2_in_robot_grip", "put_egg2_in_bowl"),
      Condition("is_not_egg2_in_bowl", "put_egg2_in_bowl")
    ]
  def setup(self):
    self.logger.debug(f"PutEgg2InBowlAction::setup")

  def initialise(self):
    self.logger.debug(f"PutEgg2InBowlAction::initialise")
  ...
  def update(self):
    ...
    robocasa.demos.singleton.general_information["previous_state"] = final_state
    robocasa.demos.singleton.general_information["previous_env"] = env
    if determine_egg_bowl(env, "is_egg2_in_bowl") == Status.SUCCESS:
      return determine_robot_egg2_grip(env, "is_egg2_not_in_robot_grip")
    return Status.FAILURE

  
  def terminate(self, new_status):
    #data = robocasa.demos.singleton.world_state["egg2_location"].name
    self.logger.debug(f"PutEgg2InBowlAction::terminate to {new_status}")


class PickEgg1FromFridgeAction(Behaviour):
  def __init__(self):
    super().__init__("pick_egg1_from_fridge")
    self.conditions = [
        Condition("is_robot_grip_not_occupied", "pick_egg1_from_fridge"),
        Condition("check_eggs_1", "pick_egg1_from_fridge"),
        Condition("is_fridge_egg1_there", "pick_egg1_from_fridge")
    ]
  def setup(self):
    self.logger.debug(f"PickEgg1FromFridgeAction::setup")

  def initialise(self):
    self.logger.debug(f"PickEgg1FromFridgeAction::initialise")
  ...
  def update(self):
    ...
    robocasa.demos.singleton.general_information["previous_state"] = final_state
    robocasa.demos.singleton.general_information["previous_env"] = env
    if determine_robot_egg1_grip(env, "is_egg1_in_robot_grip") == Status.SUCCESS:
        return determine_egg_fridge(env, "is_not_fridge_egg1_there")
    return Status.FAILURE
  
  def terminate(self, new_status):
    self.logger.debug(f"PickEgg1FromFridgeAction::terminate to {new_status}")


class PutEgg1InBowlAction(Behaviour):
  def __init__(self):
    super().__init__("put_egg1_in_bowl")
    self.conditions = [
        Condition("is_egg1_in_robot_grip", "put_egg1_in_bowl"),
        Condition("is_not_egg1_in_bowl", "put_egg1_in_bowl")
    ]
  def setup(self):
    self.logger.debug(f"PutEgg1InBowlAction::setup")

  def initialise(self):
    self.logger.debug(f"PutEgg1InBowlAction::initialise")
  ...
  def update(self):
    ...
    robocasa.demos.singleton.general_information["previous_state"] = final_state
    robocasa.demos.singleton.general_information["previous_env"] = env
    if determine_egg_bowl(env, "is_egg1_in_bowl") == Status.SUCCESS:
      return determine_robot_egg1_grip(env, "is_egg1_not_in_robot_grip")
    return Status.FAILURE
  
  def terminate(self, new_status):
    self.logger.debug(f"PutEgg1InBowlAction::terminate to {new_status}")
...
class PickEgg1FromBowlAction(Behaviour):
  def __init__(self):
    super().__init__("pick_egg1_from_bowl")
    self.conditions =  [
      Condition("is_robot_grip_not_occupied", "pick_egg1_from_bowl"),
      Condition("is_egg1_in_bowl", "pick_egg1_from_bowl")
    ]
  def setup(self):
    self.logger.debug(f"PickEgg1FromBowlAction::setup")

  def initialise(self):
    self.logger.debug(f"PickEgg1FromBowlAction::initialise")
  ...
  def update(self):
    ...
    robocasa.demos.singleton.general_information["previous_state"] = final_state
    robocasa.demos.singleton.general_information["previous_env"] = env
    if determine_egg_bowl(env, "is_not_egg1_in_bowl") == Status.SUCCESS:
      return determine_robot_egg1_grip(env, "is_egg1_in_robot_grip")
    return Status.FAILURE
  
  def terminate(self, new_status):
    self.logger.debug(f"PickEgg1FromBowlAction::terminate to {new_status}")
 

class PutEgg1InPotAction(Behaviour):
  def __init__(self):
    super().__init__("put_egg1_in_pot")
    self.conditions =  [
      Condition("is_egg1_in_robot_grip", "put_egg1_in_pot")
    ]
  def setup(self):
    self.logger.debug(f"PutEgg1InPotAction::setup")

  def initialise(self):
    self.logger.debug(f"PutEgg1InPotAction::initialise")
  ...
  def update(self):
    ...
    if robocasa.demos.singleton.world_state["is_water_boiled"] == False:
      return Status.FAILURE
    if determine_egg_pot(env, "is_egg1_in_pot") == Status.SUCCESS:
      return determine_robot_egg1_grip(env, "is_egg1_not_in_robot_grip")
    return Status.FAILURE
  
  def terminate(self, new_status):
    self.logger.debug(f"PutEgg1InPotAction::terminate to {new_status}")
   

class PickEgg2FromBowlAction(Behaviour):
  def __init__(self):
    super().__init__("pick_egg2_from_bowl")
    self.conditions =  [
      Condition("is_robot_grip_not_occupied", "pick_egg2_from_bowl"),
      Condition("is_egg2_in_bowl", "pick_egg2_from_bowl")
    ]
  def setup(self):
    self.logger.debug(f"PickEgg2FromBowlAction::setup")

  def initialise(self):
    #value = robocasa.demos.singleton.world_state["egg2_location"].name
    self.logger.debug(f"PickEgg2FromBowlAction::initialise")
  ...
  def update(self):
    ...
    robocasa.demos.singleton.general_information["previous_state"] = final_state
    robocasa.demos.singleton.general_information["previous_env"] = env
    if determine_egg_bowl(env, "is_not_egg2_in_bowl") == Status.SUCCESS:
      return determine_robot_egg2_grip(env, "is_egg2_in_robot_grip")
    
    return Status.FAILURE
  
  def terminate(self, new_status):
    #value = robocasa.demos.singleton.world_state["egg2_location"].name
    #print("robocasa.demos.singleton.world_state[egg2_location]=", value)
    self.logger.debug(f"PickEgg2FromBowlAction::terminate to {new_status}")
   

class PutEgg2InPotAction(Behaviour):
  def __init__(self):
    super().__init__("put_egg2_in_pot")
    self.conditions =  [
      Condition("is_egg2_in_robot_grip", "put_egg2_in_pot")
    ]
  def setup(self):
    self.logger.debug(f"PutEgg2InPotAction::setup")

  def initialise(self):
    self.logger.debug(f"PutEgg2InPotAction::initialise")
  ...
  def update(self):
    ...
    if robocasa.demos.singleton.world_state["is_water_boiled"] == False:
      return Status.FAILURE
    if determine_egg_pot(env, "is_egg2_in_pot") == Status.SUCCESS:
      return determine_robot_egg2_grip(env, "is_egg2_not_in_robot_grip")
    return Status.FAILURE
  
  def terminate(self, new_status):
    self.logger.debug(f"PutEgg2InPotAction::terminate to {new_status}")
...
class PickEgg2OutOfPotAction(Behaviour):
  def __init__(self):
    super().__init__("pick_egg2_out_of_pot")
    self.conditions = [
      Condition("is_robot_grip_not_occupied", "pick_egg2_out_of_pot"),
      Condition("is_egg2_in_pot", "pick_egg2_out_of_pot")
    ]
  def setup(self):
    self.logger.debug(f"PickEgg2OutOfPotAction::setup")

  def initialise(self):
    #value = robocasa.demos.singleton.world_state["egg2_location"].name
    self.logger.debug(f"PickEgg2OutOfPotAction::initialise")
  ...
  def update(self):
    ...
    robocasa.demos.singleton.general_information["previous_state"] = final_state
    robocasa.demos.singleton.general_information["previous_env"] = env
    if determine_egg_pot(env, "is_not_egg2_in_pot") == Status.SUCCESS:
      return determine_robot_egg2_grip(env, "is_egg2_in_robot_grip")
    return Status.FAILURE
  
  def terminate(self, new_status):
    #value = robocasa.demos.singleton.world_state["egg2_location"].name
    self.logger.debug(f"PickEgg2OutOfPotAction::terminate to {new_status}")




class PickEgg1OutOfPotAction(Behaviour):
  def __init__(self):
    super().__init__("pick_egg1_out_of_pot")
    self.conditions = [
      Condition("is_robot_grip_not_occupied", "pick_egg1_out_of_pot"),
      Condition("is_egg1_in_pot", "pick_egg1_out_of_pot")
    ]
  def setup(self):
    self.logger.debug(f"PickEgg1OutOfPotAction::setup")

  def initialise(self):
    self.logger.debug(f"PickEgg1OutOfPotAction::initialise")
  ...
  def update(self):
    ...
    robocasa.demos.singleton.general_information["previous_state"] = final_state
    robocasa.demos.singleton.general_information["previous_env"] = env
    if determine_egg_pot(env, "is_not_egg1_in_pot") == Status.SUCCESS:
      
      return determine_robot_egg1_grip(env, "is_egg1_in_robot_grip")
    return Status.FAILURE
  
  def terminate(self, new_status):
    self.logger.debug(f"PickEgg1OutOfPotAction::terminate to {new_status}")
...
def final_condition():
  ws = robocasa.demos.singleton.world_state
  gs = robocasa.demos.singleton.goal_state
  print("\n=== FINAL CONDITION DEBUG ===")
  print("WORLD STATE:")
  for k, v in ws.items():
      print(f"  {k}: {v}")
  print("\nGOAL STATE:")
...
def find_the_initial_world_state(action):
  ...
  determine_egg_pot(env, "is_egg1_in_pot")
  determine_egg_pot(env, "is_egg2_in_pot")
  ...
  determine_egg_fridge(env, "is_fridge_egg1_there")
  determine_egg_fridge(env, "is_fridge_egg2_there")
  determine_egg_bowl(env, "is_egg1_in_bowl")
  determine_egg_bowl(env, "is_egg2_in_bowl")
...
class Condition(Behaviour):
  def __init__(self, name, next_action=""):
  ...
  def update(self):
    ...
    if self.action != "":
      ...
      if robocasa.demos.singleton.general_information["previous_env"] == None:
        determine_egg_pot(env_, "is_egg1_in_pot")
        determine_egg_pot(env_, "is_egg2_in_pot")
        ...
        determine_egg_fridge(env_, "is_fridge_egg1_there")
        determine_egg_fridge(env_, "is_fridge_egg2_there")
        determine_egg_bowl(env_, "is_egg1_in_bowl")
        determine_egg_bowl(env_, "is_egg2_in_bowl")
...
def custom_initial_state_definition(args=None, time=None):
  ...
  elif args == None or args.time is None:
    initial_state_ = np.loadtxt(
...
def initialize_custom_goal_state(args=None, time=None):
  ...
  elif args is None or args.time is None:
    f = open('robocasa/demos/goal_state_file', 'r')
  else:
  ...
  if args != None:
    ...
    if args.action_list:
      ...
      for action in args.action_list:
        ...
        elif action == "pick_egg1_from_bowl":
          pick_egg1_from_bowl = PickEgg1FromBowlAction()
          list_of_actual_actions.extend(pick_egg1_from_bowl.conditions)
          list_of_actual_actions.append(pick_egg1_from_bowl)
        elif action == "pick_egg2_from_bowl":
          pick_egg2_from_bowl = PickEgg2FromBowlAction()
          list_of_actual_actions.extend(pick_egg2_from_bowl.conditions)
          list_of_actual_actions.append(pick_egg2_from_bowl)
        ...
        elif action == "pick_egg1_out_of_pot":
          pick_egg1_out_of_pot = PickEgg1OutOfPotAction()
          list_of_actual_actions.extend(pick_egg1_out_of_pot.conditions)
          list_of_actual_actions.append(pick_egg1_out_of_pot)   
        elif action == "pick_egg2_out_of_pot":
          pick_egg2_out_of_pot = PickEgg2OutOfPotAction()
          list_of_actual_actions.extend(pick_egg2_out_of_pot.conditions)
          list_of_actual_actions.append(pick_egg2_out_of_pot)
        elif action == "put_egg2_in_pot":
          put_egg2_in_pot = PutEgg2InPotAction()
          list_of_actual_actions.extend(put_egg2_in_pot.conditions)
          list_of_actual_actions.append(put_egg2_in_pot)   
        elif action == "put_egg1_in_pot":    
          put_egg1_in_pot = PutEgg1InPotAction()
          list_of_actual_actions.extend(put_egg1_in_pot.conditions)
          list_of_actual_actions.append(put_egg1_in_pot)
        ...
        elif action == "pick_egg1_from_fridge":
          pick_egg1_from_fridge = PickEgg1FromFridgeAction()
          list_of_actual_actions.extend(pick_egg1_from_fridge.conditions)
          list_of_actual_actions.append(pick_egg1_from_fridge)
        elif action == "pick_egg2_from_fridge": 
          pick_egg2_from_fridge = PickEgg2FromFridgeAction()
          list_of_actual_actions.extend(pick_egg2_from_fridge.conditions)
          list_of_actual_actions.append(pick_egg2_from_fridge)
        elif action == "put_egg2_in_bowl":
          put_egg2_in_bowl = PutEgg2InBowlAction()
          list_of_actual_actions.extend(put_egg2_in_bowl.conditions)
          list_of_actual_actions.append(put_egg2_in_bowl)
        elif action == "put_egg1_in_bowl":
          put_egg1_in_bowl = PutEgg1InBowlAction()
          list_of_actual_actions.extend(put_egg1_in_bowl.conditions)
          list_of_actual_actions.append(put_egg1_in_bowl)
