...
def compute_robot_base_placement_pose(env, ref_fixture, ref_object=None, offset=None):
    ...
    if not fixture_is_type(ref_fixture, FixtureType.DINING_COUNTER):
        ...
        for fxtr in ground_fixtures:
            ...
            #print("fxtr=", fxtr, " point=", point, "fxtr_name=", fxtr.name)
            if fxtr.name == "counter_corner_1_main_group_1" or fxtr.name=="fridgebottomfreezer_main_group_1":
                #print("point:", point)
    ...
    if (
        ...
        #print(f'Dist= {dist1} {dist2} {dist3} {dist4}')

        if fixture_is_type(ground_fixture, FixtureType.ISLAND):
            #print("to fixture_is_type(ground_fixture, FixtureType.ISLAND)")
    ...
    if (
        ...
        if (
            ...
            if fixture_is_type(ground_fixture, FixtureType.ISLAND):
                #print("to min(dist1, dist2, dist3, dist4) ")
                min_dist = min(dist1, dist2, dist3, dist4)
                if min_dist == dist1:
                    face_dir = 1
                elif min_dist == dist2:
...
def init_robot_base_pose(env):
    ...
    if env.init_robot_base_ref is not None:
        ...
        if env.init_robot_base_ref == FixtureType.COUNTER:
            ref_fixture = env.counter
        elif env.init_robot_base_ref == FixtureType.FRIDGE:
            ref_fixture = env.fridge
        elif env.init_robot_base_ref == FixtureType.SINK:
            ref_fixture = env.sink
        elif env.init_robot_base_ref == FixtureType.CABINET:
            ref_fixture = env.cabinet
        elif env.init_robot_base_ref == FixtureType.STOVE:
            ref_fixture = env.stove 
...
def create_obj(env, cfg):
    ...
    if cfg['mjcf_path'] is not None:
        object_kwargs['mjcf_path'] = cfg['mjcf_path']
...
def generate_random_robot_pos2(env, anchor_pos, anchor_ori, pos_dev_x, pos_dev_y):

    local_deviation = (0, -pos_dev_y*10)

    if anchor_ori[2] < math.pi/4:
        local_deviation = (0, -pos_dev_y*5)
    local_deviation = np.concatenate((local_deviation, [0.0]))
...
def set_robot_to_position2(env, global_pos):
    local_pos = np.matmul(
        T.matrix_inverse(T.euler2mat(env.init_robot_base_ori_anchor)), global_pos
    )
...
def set_robot_base2(
    ...
    with no_collision(env.sim):
        ...
        env.sim.data.qpos[
            env.sim.model.get_joint_qpos_addr("mobilebase0_joint_mobile_yaw")
        ] =  (anchor_ori[2] - env.init_robot_base_ori_anchor[2])
        ...
        sum_egg1 = diff_x_axis_egg1 + diff_y_axis_egg1 + diff_z_axis_egg1
        sum_egg2 = diff_x_axis_egg2 + diff_y_axis_egg2 + diff_z_axis_egg2
    ...
    while found_valid is not True:
        ...
        for attempt_position in range(50):
            ...
            robot_pos = generate_random_robot_pos2(
                env=env,
                anchor_pos=anchor_pos,
                anchor_ori=anchor_ori,
                pos_dev_x=cur_dev_pos_x,
            ...
            set_robot_to_position2(env, robot_pos)
    ...
    pos_1, quat_1 = env.object_placements["pot"][:2]
   
    x_w = math.cos((anchor_ori[2] - env.init_robot_base_ori_anchor[2])/2)
    x_i = 0
    x_j = 0
    x_k = math.sin((anchor_ori[2] - env.init_robot_base_ori_anchor[2])/2)

    
    env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("robot0_joint7")] = 0.6884711838378076
    env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("robot0_joint6")] = 1.4868438895865674
    env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("robot0_joint5")] = 0.04622090554898577
    env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("robot0_joint4")] = -2.2698162378501205
    env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("robot0_joint3")] = -0.03022572158656032
    env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("robot0_joint2")] = -1.02528676248207
    env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("robot0_joint1")] = -0.019099328299774208
    env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("mobilebase0_joint_torso_height")] =  -1.2131718824464208e-06
    
    env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("gripper0_right_finger_joint1")] = 0.01004152779878083
    env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("gripper0_right_finger_joint2")] = -0.00982264041773698
    ...
    if (finger_joint1 > 0.01 and finger_joint1 < 0.03) and (finger_joint2 > 0.01 and finger_joint2 < 0.03):
        ...
        if (diff_x_axis_pot<0.15 and diff_y_axis_pot < 0.15 and diff_z_axis_pot < 0.15):
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("pot_joint0")[0] + 3] = x_w
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("pot_joint0")[0] + 4] = x_i
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("pot_joint0")[0] + 5] = x_j
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("pot_joint0")[0] + 6] = x_k



            angle = (anchor_ori[2] - env.init_robot_base_ori_anchor[2])


            if (angle > math.pi/2 - 0.3 and angle<math.pi-0.3) or (angle>math.pi*3/2 - 0.3 and angle<math.pi*2 - 0.3):
                env.sim.data.qpos[
                        env.sim.model.get_joint_qpos_addr("pot_joint0")[0]
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[0]
            elif angle>math.pi - 0.3 and angle<math.pi*3/2 - 0.3:
                env.sim.data.qpos[
                        env.sim.model.get_joint_qpos_addr("pot_joint0")[0]
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[0] - 0.12
            else:
                env.sim.data.qpos[
                        env.sim.model.get_joint_qpos_addr("pot_joint0")[0]
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[0] + 0.12
            ...
            if angle < math.pi/4:
                env.sim.data.qpos[
                    env.sim.model.get_joint_qpos_addr("pot_joint0")[0]+1
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[1]
            elif  (angle>math.pi - 0.3 and angle<math.pi*3/2 - 0.3):
                env.sim.data.qpos[
                    env.sim.model.get_joint_qpos_addr("pot_joint0")[0]+1
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[1]
            
            elif angle > math.pi/2-0.3 and angle<math.pi-0.3:
                env.sim.data.qpos[
                    env.sim.model.get_joint_qpos_addr("pot_joint0")[0]+1
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[1] + 0.12
            else:
                env.sim.data.qpos[
                    env.sim.model.get_joint_qpos_addr("pot_joint0")[0]+1
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[1] - 0.12
            ...
            env.sim.data.qpos[
                env.sim.model.get_joint_qpos_addr("pot_joint0")[0]+2
            ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[2]-0.06

            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("pot_joint0")[0]
            ] = 0
            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("pot_joint0")[0]+1
            ] = 0


            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("pot_joint0")[0]+2
            ] = 0

            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("pot_joint0")[0]+3
            ] = 0
            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("pot_joint0")[0]+4
            ] = 0


            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("pot_joint0")[0]+5
            ] = 0
        elif (diff_x_axis_egg2 < 0.05 and diff_y_axis_egg2 < 0.05 and diff_z_axis_egg2 < 0.05) and (sum_egg2<sum_egg1):
            x_w = math.cos((anchor_ori[2] - env.init_robot_base_ori_anchor[2])/2)*math.cos(math.pi/4)
            x_i = -math.sin((anchor_ori[2] - env.init_robot_base_ori_anchor[2])/2)*math.sin(math.pi/4)
            x_j = math.cos((anchor_ori[2] - env.init_robot_base_ori_anchor[2])/2)*math.sin(math.pi/4)
            x_k = math.sin((anchor_ori[2] - env.init_robot_base_ori_anchor[2])/2)*math.cos(math.pi/4)

            
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg2_joint0")[0] + 3] = x_w
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg2_joint0")[0] + 4] = x_i
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg2_joint0")[0] + 5] = x_j
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg2_joint0")[0] + 6] = x_k

            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("gripper0_right_finger_joint1")] = 0.0204152779878083
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("gripper0_right_finger_joint2")] = -0.02502264041773698
            ...
            if (angle > math.pi/2 - 0.3 and angle<math.pi-0.3) or (angle>math.pi*3/2 - 0.3 and angle<math.pi*2 - 0.3):
                env.sim.data.qpos[
                        env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[0]
            elif angle>math.pi - 0.3 and angle<math.pi*3/2 - 0.3:
                env.sim.data.qpos[
                        env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[0] - 0.01
            else:
                env.sim.data.qpos[
                        env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[0] + 0.01
            ...
            if angle < math.pi/4:
                env.sim.data.qpos[
                    env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]+1
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[1]
            elif  (angle>math.pi - 0.3 and angle<math.pi*3/2 - 0.3):
                env.sim.data.qpos[
                    env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]+1
                ...
                env.sim.data.qpos[
                    env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]+1
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[1] + 0.01
            else:
                env.sim.data.qpos[
                    env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]+1
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[1] - 0.01
            ...
            env.sim.data.qpos[
                env.sim.model.get_joint_qpos_addr("egg2_joint0")[0]+2
            ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[2]

            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg2_joint0")[0]
            ] = 0
            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg2_joint0")[0]+1
            ] = 0


            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg2_joint0")[0]+2
            ] = 0


            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg2_joint0")[0]+3
            ] = 0
            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg2_joint0")[0]+4
            ] = 0


            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg2_joint0")[0]+5
            ] = 0   
        elif (diff_x_axis_egg1 < 0.05 and diff_y_axis_egg1 < 0.05 and diff_z_axis_egg1 < 0.05) :
            x_w = math.cos((anchor_ori[2] - env.init_robot_base_ori_anchor[2])/2)*math.cos(3*math.pi/4) - math.sin((anchor_ori[2] - env.init_robot_base_ori_anchor[2])/2)*math.sin(3*math.pi/4)
            x_i = 0
            x_j = 0
            x_k = math.sin((anchor_ori[2] - env.init_robot_base_ori_anchor[2])/2)*math.cos(3*math.pi/4) + math.cos((anchor_ori[2] - env.init_robot_base_ori_anchor[2])/2)*math.sin(3*math.pi/4)


            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg1_joint0")[0] + 3] = x_w
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg1_joint0")[0] + 4] = x_i
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg1_joint0")[0] + 5] = x_j
            env.sim.data.qpos[env.sim.model.get_joint_qpos_addr("egg1_joint0")[0] + 6] = x_k
            ...
            if (angle > math.pi/2 - 0.3 and angle<math.pi-0.3) or (angle>math.pi*3/2 - 0.3 and angle<math.pi*2 - 0.3):
                env.sim.data.qpos[
                        env.sim.model.get_joint_qpos_addr("egg1_joint0")[0]
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[0]
            elif angle>math.pi - 0.3 and angle<math.pi*3/2 - 0.3:
                env.sim.data.qpos[
                        env.sim.model.get_joint_qpos_addr("egg1_joint0")[0]
            ...
            if angle < math.pi/4:
                env.sim.data.qpos[
                    env.sim.model.get_joint_qpos_addr("egg1_joint0")[0]+1
                ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[1]
            elif  (angle>math.pi - 0.3 and angle<math.pi*3/2 - 0.3):
                env.sim.data.qpos[
                    env.sim.model.get_joint_qpos_addr("egg1_joint0")[0]+1
            ...
            env.sim.data.qpos[
                env.sim.model.get_joint_qpos_addr("egg1_joint0")[0]+2
            ] =  env.sim.data.get_site_xpos("gripper0_right_grip_site")[2]

            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg1_joint0")[0]
            ] = 0
            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg1_joint0")[0]+1
            ] = 0


            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg1_joint0")[0]+2
            ] = 0

            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg1_joint0")[0]+3
            ] = 0
            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg1_joint0")[0]+4
            ] = 0


            env.sim.data.qvel[
                env.sim.model.get_joint_qvel_addr("egg1_joint0")[0]+5
            ] = 0  
